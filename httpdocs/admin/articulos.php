	...
	<?php
		if($_GET["delete"]){
			...
			if($response > 0)
				...
				echo "<h4 class='alert_error'>El art&iacute;culo no se pudo eliminar. Verifique que no tenga im&aacute;genes o que no pertenezca al slideshow</h4>";
		}
		if( isset($_POST['name']) && !isset($_POST['id'])){
			...
			$sql = "insert into products(name,title,description,tags,dimensions,price,cat_id,created_at, total, availables) values('$name','$title','$str_description','$tags','$dimensions','$price',$cat, NOW(), $total, $availables)";			
			mysql_query($sql);
			...
			$sql = "update products set name = '$name', title = '$title', description = '$str_description', tags = '$tags', dimensions = '$dimensions', price = $price , cat_id = $cat_id, total=$total, availables=$availables WHERE id = $id";			
			mysql_query($sql);
